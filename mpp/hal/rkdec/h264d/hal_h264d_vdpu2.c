...
static void myy_dump_regs(
	...
	H264dVdpuRegs_t const * __restrict const p_regs)
{
...
MPP_RET vdpu2_h264d_gen_regs(void *hal, HalTaskInfo *task)
{
    ...
    FUN_CHECK(ret = set_pic_regs(p_hal, (H264dVdpuRegs_t *)p_hal->regs));
	...
	myy_dump_frame_and_regs(p_hal, (H264dVdpuRegs_t *)p_hal->regs);
	ret = MPP_OK;
__RETURN:
__FAILED:
    return ret;
